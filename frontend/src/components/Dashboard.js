...
const Dashboard = () => {
  ...
  const prevCurrency = () => {
    ...
    setCurrentCurrencyIndex(
      (prev) => (prev - 1 + currencyEntries.length) % currencyEntries.length,
    );
  };
    ...
    <div>
          ...
          >
            ...
            <div style={{ opacity: 0.9, marginBottom: "0.5rem" }}>
              {currencyEntries.length > 1
                ? `Balance (${currentCurrencyIndex + 1}/${currencyEntries.length})`
                : "Total Balance"}
            </div>
      ...
      <div className="card">
        ...
        {stats.recent_accounts && stats.recent_accounts.length > 0 ? (
          ...
          >
            No accounts found.{" "}
            <a href="/create-account" style={{ color: "#667eea" }}>
